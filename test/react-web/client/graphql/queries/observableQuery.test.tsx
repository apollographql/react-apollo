...
describe('[queries] observableQuery', () => {
  // observableQuery
  xit(
    'will recycle `ObservableQuery`s when re-rendering the entire tree',
    done => {
      const query = gql`
        query people {
          allPeople(first: 1) {
  ...
  xit(
    'will not try to refetch recycled `ObservableQuery`s when resetting the client store',
    done => {
      const query = gql`
        query people {
          allPeople(first: 1) {
      ...
      finish = () => {
        ...
        expect(link.query).toHaveBeenCalledTimes(1);
      ...
      const queryObservable1 = (client as any).queryManager.observableQueries[
        '1'
      ].observableQuery;
  ...
  xit(
    'will refetch active `ObservableQuery`s when resetting the client store',
    () => {
      const query = gql`
        query people {
          allPeople(first: 1) {
      ...
      expect(link.query).toHaveBeenCalledTimes(2);
    },
  );

  xit(
    'will recycle `ObservableQuery`s when re-rendering a portion of the tree',
    done => {
      const query = gql`
        query people {
          allPeople(first: 1) {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          const queryObservable3 = (client as any).queryManager
            .observableQueries['1'].observableQuery;
  ...
  xit(
    'will not recycle parallel GraphQL container `ObservableQuery`s',
    done => {
      const query = gql`
        query people {
          allPeople(first: 1) {
