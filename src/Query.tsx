...
> {
  ...
  private getQueryResult = (client: ApolloClient<object>): QueryResult<TData, TVariables> => {
    let result = {
    ...
    if (this.props.skip) {
      ...
      if (loading) {
        const previousData =
          this.previousResult ? this.previousResult.data : {};
  ...
  private renderData(context: ApolloContextValue): React.ReactNode {
    ...
    const finish =
      () => this.props.children(this.getQueryResult(client));
    if (context && context.renderPromises) {
