...
export class MockedProvider extends React.Component<any, any> {
  ...
  render() {
    return (
      <ApolloProvider
        client={this.client || this.props.client}
...
export class MockNetworkInterface implements NetworkInterface {
  ...
  public query(request: Request) {
    return new Promise((resolve, reject) => {
      ...
      if (
        !this.mockedResponsesByKey[key] ||
        this.mockedResponsesByKey[key].length === 0
      ) {
        throw new Error(
          'No more mocked responses for the query: ' + print(request.query),
        );
      }
